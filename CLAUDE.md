...
**Status:** Phase 1 done — Backend: `apps/accounts` with email-based custom `User`, JWT auth (`login`/`refresh`/`logout`+blacklist), `profile`, password change + reset (Celery email), `LoginHistory`, `rbac.py` (`OPERATIONAL_PERMISSIONS` + `ROLE_PERMISSIONS` for Admin/Manager/Cashier/Warehouse) materialized via `ensure_role_groups()` on a `post_migrate` hook, `permissions.py:HasOperationalPermission` + `require()`. Frontend: `frontend/` scaffolded (Vite + React + TS, React Router, TanStack Query, Zustand auth store, Tailwind + shadcn/ui), app shell (sidebar/topbar), login page, `ProtectedRoute`, stub routes per domain area, live serialized-units panel on the new fetch client. `make test` (42 tests), `ruff check`, `spectacular --validate`, frontend `eslint` + `tsc -b && vite build`, and a full `docker compose up` (backend + celery-worker + celery-beat + frontend + postgres + redis) all pass.
**Next up:** Phase 2 — Catalog (Product / Variant / Attributes / SKU) (see `HEXAGARE_BUILD_PROMPTS.md`).
**Completed phases:** Phase 0, Phase 1.
**Notes / deviations from the plan:** (1) S3/local storage switch uses Django 5.1+ `STORAGES` (keyed on `USE_S3`), not the removed `DEFAULT_FILE_STORAGE` — same behaviour. (2) `manage.py`/`wsgi.py`/`asgi.py` keep `DJANGO_SETTINGS_MODULE=hexagare.settings`; env selection happens inside `settings/__init__.py` via `DJANGO_ENV`; `make test` passes `DJANGO_ENV=test`. (3) `docker-compose.yml` sets `name: hexagare` and backend host port is `${BACKEND_PORT:-8000}`. (4) Added `apps/common` health endpoint at `/api/v1/health/` and a repo `.gitignore` (not in the prompt). (5) **DB volume reset required going into Phase 2's first `migrate` only if coming from a pre-Phase-1 volume** — the custom `User` landed in Phase 1 (`accounts.0001_initial`); `docker compose down -v` was run during Phase 1. (6) Login identifier is **email**, not username (custom `UserManager`, `USERNAME_FIELD="email"`). (7) Operational permissions are anchored to an unmanaged `accounts.OperationalPermission` model's content type (keeps them separate from model CRUD perms; makes stale-perm pruning safe). (8) Auth/token responses keep `rest_framework_simplejwt`'s `{access,refresh}` shape (login adds `user`); the list/error envelopes apply to lists and errors only. (9) Frontend↔backend uses a **Vite dev proxy** (`/api` → `backend:8000`), so no `django-cors-headers`. (10) The "existing serialized-units panel" in the prompt didn't exist (no `frontend/` before this phase) — built fresh, pointed at the provisional Phase 3 route `/api/v1/products/serialized-units/`, renders a friendly "not available yet" state on 404/501. (11) ADR-003 is the role-permission model; the Phase 7 prompt's stray "ADR-003" reference for sales channels will renumber. (12) Chrome DevTools MCP not connected — frontend verified manually (dev server serves SPA + transforms TSX, Vite `/api` proxy works, full login/refresh/logout/profile flow exercised over HTTP).
